...
function login() {
    ...
    const loginusers = async (e) => {
        e.preventDefault();
                ...
                const res =await axios.post('/api/admin/Loginadminapi',
                {
                    email: email,
                    password: password
                }) 
                
                if((await res).data.message === "admin logged in")
                {
                  localStorage.setItem('admintoken',(await res).data.admintoken);
  ...
  return (
    ...
    <Form.Group className={styles.formgroup}>
     ...
     <Form.Control className={styles.input} type="password" placeholder="Enter password" required value={password} onChange={(e) =>setPassword(e.target.value)}></Form.Control>
